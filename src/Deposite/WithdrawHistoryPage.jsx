...
function WithdrawHistoryPage() {
   ...
   const { setloading} = useAuth();
      ...
      const fetchWithdrawHistory = async () => {
        ...
        try {
          const res = await fetchData({
            url: `/api/v1/user/payment/withdraw-history?userId=${userId}`,
          });
          setData(res.data || []);
          setloading(false)
        } catch (error) {
    ...
    const filteredData = data.filter((item) =>
      `${item.amount}${item.coin}${item.toAddress}${item.status}${item.txnId}`
        .toLowerCase()
  ...
  return (
      ...
      <div className="p-4 md:p-6">
        <h2 className="text-xl md:text-2xl font-bold text-center text-[#0671FF] mb-4">
          Withdraw History
        </h2>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 text-sm">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <input
              type="text"
              placeholder="Search coin/status/txn"
              value={searchTerm}
              onChange={(e) => {
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-900 text-[#0671FF] whitespace-nowrap">
              <tr>
                ...
                {/* <th className="px-4 py-2 border-b border-gray-700 text-left">Coin</th> */}
                ...
                {/* <th className="px-4 py-2 border-b border-gray-700 text-left">Txn ID</th> */}
              ...
              {paginatedData.length > 0 ? (
                paginatedData.map((item, index) => (
                  <tr key={item._id} className="hover:bg-gray-800 border-b border-gray-800">
                    ...
                    {/* <td className="px-4 py-2 break-all text-xs text-gray-300">{item.txnId}</td> */}
