...
import IncomeNav from '../Income/IncomeNav';
...
function Maintofund() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setResponseMsg('✅ Successfully transferred!');
      } else {
        setResponseMsg('❌ Failed to transfer!');
      }
      setloading(false);
    } catch (error) {
  ...
  return (
    <div className="mt-[80px] min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      <FundNav /> 
      <div className="flex mt-10 justify-center  bg-gradient-to-b k px-4">
        <div className="bg-gray-900 p-8 rounded-2xl shadow-2xl w-full max-w-lg border border-gray-800">
          ...
          <div className="flex justify-between bg-gray-800 rounded-lg p-4 mb-4 text-sm">
            <div>
              ...
              <p className="text-white font-bold text-lg">
                ${Number(Data?.walletBalance).toFixed(2) || 0}
              </p>
            </div>
            <div>
              <p className="text-gray-400">Fund Balance</p>
              <p className="text-white font-bold text-lg">
                ${Number(Data?.fundBalance).toFixed(2) || 0}
              </p>
            </div>
          </div>
          ...
          <p className="text-red-400 text-center text-sm mb-6">
            ⚠️ Transaction charge: <span className="font-semibold">5%</span>
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-lg  text-white font-bold hover:bg-lime-400 transition disabled:opacity-50"
          ...
          {responseMsg && (
            <p
              className={`text-center mt-4 font-medium ${
                responseMsg.includes('✅') ? 'text-green-400' : 'text-red-400'
              }`}
            >
              {responseMsg}
            </p>
