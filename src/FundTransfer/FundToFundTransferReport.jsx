...
function FundToFundTransferReport() {
  ...
  useEffect(() => {
    const fetchTransferHistory = async () => {
      setloading(true);
      try {
        const res = await fetchData({ url: `/api/v1/user/payment/history/fund-to-fund/${userId}` });
        setData(res.data || []);
        setloading(false);
      } catch (error) {
        console.error('Error fetching Fund-to-Fund Transfer History:', error);
        setloading(false);
      }
    };
  ...
  const filteredData = data.filter((item) =>
    `${item.fromUserId}${item.toUserId}${item.amount}${item.receivedAmount}${item.fee}`.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white">
      <div className="p-4 md:p-6">
        <h2 className="text-xl md:text-2xl font-bold text-center text-white mb-4">Fund to Fund Transfer History</h2>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 text-sm">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <input type="text" placeholder="Search by UserId/Amount" value={searchTerm}
            ...
            <tbody>
              {paginatedData.length > 0 ? paginatedData.map((item, index) => (
                <tr key={item._id} className="hover:bg-gray-800 border-b border-gray-800">
                  ...
                  <td className="px-4 py-2">{item.fromUserId}</td>
                  <td className="px-4 py-2">{item.toUserId}({item.toUserName})</td>
                ...
                <tr><td colSpan="7" className="text-center py-4 text-gray-400">No fund-to-fund records found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
export default FundToFundTransferReport;
