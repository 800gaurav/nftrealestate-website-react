...
// setloading(true)
        const res = await fetchData({
          url: `/api/v1/user/payment/history/wallet-to-fund/${userId}`,
        });
        setData(res.data || []);
        // setloading(false)
      } catch (error) {
  ...
  const filteredData = data.filter((item) =>
    `${item.from}${item.to}${item.amount}${item.receivedAmount}${item.fee}`
      .toLowerCase()
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 text-sm">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <input
              type="text"
              placeholder="Search from/amount/fee"
              value={searchTerm}
              onChange={(e) => {
        ...
        <div className="overflow-x-auto border border-gray-700 rounded">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-900 whitespace-nowrap"
