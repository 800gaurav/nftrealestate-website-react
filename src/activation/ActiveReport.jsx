...
function ActiveReport() {
  ...
  const fetchNFTData = async () => {
    try {
      const res = await fetchData({
        url: `/api/v1/user/nft/purchase/get-user-nft/${userId}`
      });
  ...
  return (
    <div className="bg-color-1">
      <Usernav />
      <div className="min-h-screen mt-15 bg-[#0f0f1a] text-white p-4 md:p-6">
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {data.length > 0 ? (
                data.map((item, index) => {
                  const profitPercent =
                    item.price > 0 ? ((item.profitEarned || 0) / item.price) * 100 : 0;
                    ...
                    <tr key={item._id} className="hover:bg-gray-800">
                      ...
                      <td
                        className={`px-4 py-3 font-bold ${
                          profitPercent >= 0 ? 'text-green-400' : 'text-red-500'
                        }`}
                      >
